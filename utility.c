// Title:  myshell.c
// name :  Mariam Haboush , Roba Murtaja ,Zaina Alashi
// ID   :  220211422 ,220211110 , 220210482
...
int Command_exec (char **args, const Redirect *Inputs, const Redirect *Outputs, int *states) {
	...
	else if (!strcmp(args[0], "clr")   ||   !strcmp(args[0], "clear")) {	// "clear" command	// system("clear");
		if (output_num == 0)
...
int Command_dir (char **args, const Redirect *Inputs, int *states) {
    ...
    // handle input redirection (e.g., dir < a.txt)
    if (states[4]) {
        if (args[1])
    ...
    if (args[1]) {
        ...
        strcpy(dirpath, getenv("HOME")); // default to home directory
    }
    ...
    while ((entry = readdir(pdir)) != NULL) {
        if (entry->d_name[0] != '.' && count < MAX_FILES) {
            filenames[count] = strdup(entry->d_name);  // allocate copy
            count++;
        }
    }
    closedir(pdir);
    ...
    // print filenames in grid format (6 per line)
    for (int i = 0; i < count; i++) {
        printf("%-15s", filenames[i]);  // fixed width
        if ((i + 1) % 6 == 0)
            printf("\n");
    }
    if (count % 6 != 0)
...
int Command_shell (FILE *inputfile, const Redirect *Outputs, const int *states) {
	...
	bat_num = 0;
	do {	/* keep reading input until "quit" command or eof of redirected input */ /* get command line from input */
		if (inputfile == stdin && Outputs == NULL)
			fprintf(stderr, "\n%s@%s:%s$: ", getenv("USERNAME"), getenv("USERNAME"), getenv("PWD"));	// write prompt
